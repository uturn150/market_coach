...
def main():
    ...
    for name, cls, src, sub in (("MACDStrategy (5m)", rp.MACDStrategy, "min", None), ("MACDStrategy_crossed (5m)", rp.MACDStrategyCrossed, "min", None), ("Quickie (5m)", rp.Quickie, "min", None), ("Simple (5m)", rp.Simple, "min", None),
                                ("Scalp (1m, 6 coins)", rp.Scalp, "min", 6), ("SmoothScalp (1m, 6 coins)", rp.SmoothScalp, "min", 6)):
        if src == "min":
    ...
    json.dump(out, open(os.path.join(BASE, "logs", "refports_benchmark5.json"), "w"), indent=1)
